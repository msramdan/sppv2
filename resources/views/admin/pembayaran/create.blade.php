...
@section('content')
    <section class="section" v-cloak>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-8 mx-auto">
                                    <autocomplete :search="search" placeholder="Masukkan NIS atau Nama Siswa/i "
                                        aria-label="Search Wikipedia" :get-result-value="getResultValue"
                                        @submit="handleSubmit"></autocomplete>
                                </div>
                            </div>
                            <!-- /.card-row -->
                            <div class="row mt-5" style="min-height: 185px" v-if="dataSiswa">
                                ...
                                <div class="col-md-2 mb-4">
                                    <div class="user-item">
                                        <img alt="image" height="128px" width="128px" :src="'../img/siswa/' + siswa.foto"
                                            class="img-fluid">
                                        <div class="user-details">
                                            ...
                                            <div class="user-cta">
                                                <button class="btn btn-primary follow-btn"
                                                    ...
                                                    data-unfollow-action="alert('user5 unfollowed');">@{{ siswa . status }}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div id="accordion">
                                        <div class="accordion" v-for="(row, p) in tagihan">
                                            <div class="accordion-header collapsed" role="button" data-toggle="collapse"
                                                :data-target="'#panel-body-'+row.id" aria-expanded="false">
                                                <h4>
                                                    @{{ row . jenis_pembayaran . nama_pembayaran }}
                                                    @{{ row . jenis_pembayaran . tipe }}
                                                </h4>
                                            </div>
                                            <div class="accordion-body collapse" :id="'panel-body-'+row.id"
                                                data-parent="#accordion" style="">
                                                <div class="row">
                                                    <div class=""
                                                        v-bind:class="{ 'col-md-8 col-12': row.jenis_pembayaran.tipe ==='bebas', 'col-md-4 col-6': row.jenis_pembayaran.tipe ==='bulanan' }"
                                                        v-for="(oke, index) in row.tagihan_detail">

                                                        <div class="card">
                                                            <div class="card-body">
                                                                <div v-if="row.jenis_pembayaran.tipe === 'bulanan'"
                                                                    class="small">
                                                                    <strong>@{{ row . jenis_pembayaran . harga }}</strong>
                                                                    <br>
                                                                    @{{ oke . keterangan }}
                                                                </div>
                                                                <div v-else>
                                                                    ...
                                                                    <div class="d-flex justify-content-between">
                                                                        ...
                                                                        <strong
                                                                            class="text-success">Rp.@{{ oke . total_bayar }}</strong>
                                                                    </div>
                                                                    <div class="d-flex justify-content-between">
                                                                        <span>Sisa</span>
                                                                        <strong
                                                                            class="text-warning">Rp.@{{ oke . sisa }}</strong>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div v-else>
                                                                <button class="btn btn-warning btn-sm btn-block"
                                                                    v-if="submit[oke.id]" disabled>
                                                                    <i class="fas fa-check    "></i>
                                                                    Bayar
                                                                </button>
                                                                <div v-else>
                                                                    <button v-if="row.jenis_pembayaran.tipe === 'bulanan'"
                                                                        @click="addToCart(oke.id, row.jenis_pembayaran.harga, row.jenis_pembayaran.nama_pembayaran, oke.keterangan, row.jenis_pembayaran.tipe)"
                                                                        class="btn btn-sm btn-block btn-light">
                                                                        @{{ submit[oke . id] ? 'Loading...' : 'Bayar' }}
                                                                    </button>

                                                                    <button v-else
                                                                        @click="bayarAngsuran(oke.id, oke.sisa, row.jenis_pembayaran.nama_pembayaran, oke.keterangan)"
                                                                        class="btn btn-sm btn-block btn-light">
                                                                        Bayar
                                                                    </button>
                                                                </div>
                                    ...
                                    <div class="card card-info pembayaranDetail">
                                        ...
                                        <div class="card-body">
                                            <div class="d-flex justify-content-between">
                                                ...
                                                <div v-if="submitCart" class="spinner-border text-danger spinner-border-sm"
                                                    role="status">
                                                    ...
                                                    <div class="d-flex justify-content-between border-bottom py-4"
                                                        ...
                                                        <div>
                                                            ...
                                                            <button class="btn text-danger"
                                                                @click="deleteItem(row.rowId, row.id)"><i
                                            ...
                                            <button class="btn btn-primary btn-block" v-if="cartTotal <= 0"
                                                disabled>Lanjutkan Pembayaran</button>
                                            <button v-else class="btn btn-primary btn-block"
                                                @click="showModal = true">Lanjutkan Pembayaran</button>
                                        </div>
                                    </div>
                                </div>
                                <!-- /.col-md -->
                            ...
                            <div class="row mt-1" style="min-height: 185px" v-else>
                                <div class="col-md-6 mx-auto">
                                    <img class="w-75" style="opacity: 0.3"
                                        src="{{ asset('/img/undraw_file_searching_duff.png') }}" alt="">
                                </div>
                            </div>
                            <!-- /.card-body -->
                        </div>
        ...
        <div class="modal fade show" style="z-index: 9998" id="pembayaranModal" tabindex="-1" role="dialog"
                            ...
                            <div class="modal-content bg-light text-dark shadow">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="button" class="btn btn-primary btn-block"
                                        @click="storePembayaran(siswa.id)">
                            ...
                            <div class="modal-content bg-light text-dark shadow">
                                <div class="modal-body">
                                    <div class="form-group">
                                        ...
                                        <input type="number" min="1" :max="tmp.harga" v-model="tmp.harga"
                                            class="form-control" name="" id="" :autofocus="true">
                                        <div v-if="showError">
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-danger mt-2 btn-block"
                                        @click="showModalAngsuran = false">Close</button>
